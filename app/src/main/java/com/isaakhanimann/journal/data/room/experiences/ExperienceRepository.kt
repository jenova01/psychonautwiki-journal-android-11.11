...
class ExperienceRepository @Inject constructor(private val experienceDao: ExperienceDao) {
    ...
    suspend fun getLastUseDate(substanceName: String): LocalDateTime? =
        experienceDao.getLastUseDate(substanceName)?.let { instant ->
            LocalDateTime.ofInstant(instant, ZoneId.systemDefault())
        }

    suspend fun calculatePersonalizedTolerance(substanceName: String): Pair<Int, Int> {
        val ingestions = getSortedIngestionsFlow(substanceName, 100).firstOrNull() ?: return Pair(30, 14)
        
        if (ingestions.size < 2) return Pair(30, 14)
        
        val intervals = mutableListOf<Long>()
        for (i in 1 until ingestions.size) {
            val interval = ChronoUnit.DAYS.between(
                LocalDateTime.ofInstant(ingestions[i].time, ZoneId.systemDefault()),
                LocalDateTime.ofInstant(ingestions[i-1].time, ZoneId.systemDefault())
            )
            intervals.add(interval)
        }
        
        // Calcula a média dos intervalos para determinar a tolerância
        val averageInterval = intervals.average().toInt()
        
        // Ajusta os valores de tolerância baseado nos intervalos
        val zeroTolerance = max(30, averageInterval)
        val halfTolerance = max(14, averageInterval / 2)
        
        return Pair(zeroTolerance, halfTolerance)
    }

    suspend fun getCustomUnitsSorted(substanceName: String): List<CustomUnit> =
        experienceDao.getCustomUnitsSorted(substanceName)
}